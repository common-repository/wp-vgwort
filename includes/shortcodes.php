 ...
 */
 class WPVGW_Shortcodes
 {
     ...
     {
         $attributes = shortcode_atts(array( 'post_id' => null, 'formatted' => true, 'decimals' => 0, 'round' => false, 'round_half' => false, 'page_count_divisor' => 1800, 'text' => $this->postStatsTemplate, 'type' => 'character-count', ), $attributes);
         ...
         if ($post === null) {
             return __('Die ID der Seite ist ungültig.', WPVGW_TEXT_DOMAIN);
         }
         if (!$this->markersManager->is_post_type_allowed($post->post_type) || !$this->markersManager->is_user_allowed(intval($post->post_author))) {
             return __('Seitenautor oder Seitentyp nicht zugelassen.', WPVGW_TEXT_DOMAIN);
         }
         ...
         if ($postExtras) {
             ...
             return __('Keine Zeichenanzahl für die Seite gefunden.', WPVGW_TEXT_DOMAIN);
         }
         switch ($type) { case 'standard-page-count': if ($formatted) {
             return esc_html(number_format_i18n($this->round($standardPageCount, $decimals, $roundHalf), $decimals));
         } return $round ? $this->round($standardPageCount, $decimals, $roundHalf) : $standardPageCount; case 'custom-page-count' : if ($formatted) {
             return esc_html(number_format_i18n($this->round($customPageCount, $decimals, $roundHalf), $decimals));
         } return $round ? $this->round($customPageCount, $decimals, $roundHalf) : $customPageCount; case 'text': if ($formatted) {
             return esc_html(sprintf($text, number_format_i18n($characterCount), number_format_i18n($this->round($standardPageCount, $decimals, $roundHalf), $decimals), number_format_i18n($this->round($customPageCount, $decimals, $roundHalf), $decimals)));
         } return $round ? esc_html(sprintf($text, $this->round($characterCount, $decimals, $roundHalf), $this->round($standardPageCount, $decimals, $roundHalf), $this->round($customPageCount, $decimals, $roundHalf))) : esc_html(sprintf($text, $characterCount, $standardPageCount, $customPageCount)); default: if ($formatted) {
     ...
     private function round($number, $number_of_decimals, $round_half = false)
     {
         ...
         $decimalShift = 10 ** ($number_of_decimals + 1);
